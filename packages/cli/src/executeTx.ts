...
export async function executeTx({
    ...
    builder: MethodsBuilder<Advensum, AllInstructions<Advensum>>
    print?: string;
    simulate?: boolean;
}) {
    if (print) {
        ...
        switch (print) {
            ...
            case 'legacy':
                ...
                break;
            case 'version0':
                const txv0 = new VersionedTransaction(MessageV0.compile({
    ...
    if (simulate) {
        ...
        const tx = await builder.transaction();
        for (const k of tx.instructions[1].keys) {
        ...
        console.log(JSON.stringify(
            (await simulateTransaction(provider.connection, tx)).value));
    }
    if (!print && !simulate) {
        await builder.rpc();
    }
